...
function Top() {
  ...
  const createFollow = () => {
    supabase
      .from('FOLLOW')
      .insert([
        { following_user_id: userId, user_id: userData.userId },
      ])
      .select()
      .then((response) => {
        if (!response.error) {
  ...
  const updateIncrementUserFollowingCount = () => {
    supabase.rpc('increment_followers_count', { param: userId })
      .then((response) => {
        if (!response.error) {
          setIsFollowing(true);
        }
  ...
  const updateDecrementUserFollowingCount = () => {
    supabase.rpc('decrement_followers_count', { param: userId })
      .then((response) => {
        if (!response.error) {
    ...
    <TopWrapper>
      <LeftWrapper>
        <LeftContainer>
          ...
          {
            userData.userId !== userId ? (<FollowButton onClick={handleFollow}>
